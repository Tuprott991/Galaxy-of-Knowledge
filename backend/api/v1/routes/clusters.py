...
async def get_cluster_papers(
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            if limit:
                query += " LIMIT $2"
                papers = await conn.fetch(query, cluster_id, limit)
            else:
                papers = await conn.fetch(query, cluster_id)
            
            if not papers:
                # Check if cluster exists at all
                cluster_count = await conn.fetchval("SELECT COUNT(*) FROM paper WHERE cluster = $1", cluster_id)
...
async def get_clusters_summary():
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            # Total statistics
            total_clusters = await conn.fetchval("SELECT COUNT(DISTINCT cluster) FROM paper WHERE cluster IS NOT NULL")
            total_clustered_papers = await conn.fetchval("SELECT COUNT(*) FROM paper WHERE cluster IS NOT NULL")
            ...
            clusters_data = [
                {
                    ...
                    "papers_with_coordinates": row['papers_with_coords'],
                    "centroid": {
                        "x": float(row['avg_x']) if row['avg_x'] else None,
                        "y": float(row['avg_y']) if row['avg_y'] else None,
                        "z": float(row['avg_z']) if row['avg_z'] else None
                    }
                }
...
async def get_cluster_topic(cluster_id: str):
    """
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            results = await conn.fetch("""
                ...
                FROM paper 
                WHERE cluster = $1 AND title IS NOT NULL
                ORDER BY paper_id
            """, cluster_id)
...
async def get_all_cluster_topics(
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            if limit:
                query += " LIMIT $2"
                results = await conn.fetch(query, min_papers, limit)
            else:
                results = await conn.fetch(query, min_papers)
            ...
            for row in results:
                cluster_id = row['cluster']
                paper_count = row['paper_count']
                titles = row['titles']  # PostgreSQL array
